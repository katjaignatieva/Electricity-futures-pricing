classdef ParticleFilter_SVJDILRL < handle
  properties
  ...
  methods
    function self = ParticleFilter_SVJDILRL(model, nb_particles)
      if nargin > 1
        self.nb_particles = nb_particles;
      end
      self.model          = model;
      self.getParticles();
    end % ParticleFilter_SVJDILRL
    ...
    function [] = getParticles(self)
      ...
      switch self.model.name
        case 'SVJDILRL'
          ...
          self.particles.Wmu        = normrnd(0,1,self.nb_particles,min(self.max_length,T));
          ...
          self.particles.Zlambda    = unifrnd(0,1,self.nb_particles,min(self.max_length,T));

          self.particles.u_resampl  = unifrnd(0,1,T,3);
        otherwise
          error('Model not implemented in the ParticleFilter class')
      end
    end % getParticles
    
    function [logLikelihood, muf, vf, lambdaf, Npf, Nnf, Jpf, Jnf, Jvf, Jlambdaf] = logLikelihood(self)
      tic();
      ...
      mu            = pv.mubar.*ones(self.nb_particles,1);
      v             = pv.vbar.*ones(self.nb_particles,1);
      lambda        = pv.lambdanbar.*ones(self.nb_particles,1);

      for dt = 1:T
        [weights, mu, v, lambda, Np, Nn, Jp, Jn, Jv, Jlambda] = self.updateState(pv, dt, mu, v, lambda);
        ...
        muf(dt)       = mu'*normalizedweights;
        vf(dt)        = v'*normalizedweights;
        lambdaf(dt)   = lambda'*normalizedweights;
        ...
        Jlambdaf(dt)  = Jlambda'*normalizedweights;
        ...
        switch self.resampling
          case 'MalikPitt'
            [mu, v, lambda] = self.MalikPitt(dt, weights, mu, v, lambda);
          case 'Stratified'
            [mu, v, lambda] = self.Stratified(dt, weights, mu, v, lambda);
          otherwise
    ...
    function [munew, vnew, lambdanew] = Stratified(self, dt, weights, mu, v, lambda)
    ...
    function [munew, vnew, lambdanew] = MalikPitt(self, dt, weights, mu, v, lambda)
      Vnew = NaN(self.nb_particles,3);
      V = [mu, v, lambda];
      ...
      for dp = 1:self.nb_partitions
        ...
        if isscalar(ix)
          ...
          [V2_sorted, I] = sort(V(ix,2));
          norm_weights_tmp = norm_weights(ix(I))./sum(norm_weights(ix(I)));
          [r,ustar] = getrandustar_cpp(norm_weights_tmp,PartitionSize,self.particles.u_resampl(dt,2));
          Vnew(((dp-1)*PartitionSize+1):(dp*PartitionSize),2) = V2_sorted(r) + ustar.*(V2_sorted(min(length(ix),r+1)) - V2_sorted(max(r,1)));

          [V3_sorted, I] = sort(V(ix,3));
          norm_weights_tmp = norm_weights(ix(I))./sum(norm_weights(ix(I)));
          [r,ustar] = getrandustar_cpp(norm_weights_tmp,PartitionSize,self.particles.u_resampl(dt,2));
          Vnew(((dp-1)*PartitionSize+1):(dp*PartitionSize),3) = V3_sorted(r) + ustar.*(V3_sorted(min(length(ix),r+1)) - V3_sorted(max(r,1)));
        end
      end
      munew = Vnew(:,1);
      vnew = Vnew(:,2);
    ...
    function [weights, mu, v, lambda, Np, Nn, Jp, Jn, Jv, Jlambda] = updateState(self, pv, dt, mu_old, v_old, lambda_old)
      h           = self.model.h;
      ...
      Nn          = self.particles.Nn(:,cur_step) < lambda_old*h;
      ...
      Zlambda     = zeros(self.nb_particles,1);
      Zlambda(Np) = expinv(self.particles.Zlambda(Np,cur_step),pv.nulambda);
      ...
      averlambda  = lambda_old + pv.kappalambda.*(pv.lambdanbar - lambda_old).*h;
      lambdam     = max( 0, averlambda );
      lambda      = lambdam + Jlambda;

      avermu      = mu_old + pv.kappamu.*(pv.mubar - mu_old).*h;
      varmu       = pv.sigmamu.^2.*h;
      mu          = avermu + sqrt(varmu).*self.particles.Wmu(:,cur_step);

      % Step 3: Compute the expected value and variance of x based on
      % particles
      xtm1        = self.model.series(max(dt-1,1));

      averx       = xtm1 + pv.kappax.*(mu_old - xtm1)*h + ...
      ...
      if self.model.use_futures && height(self.model.futures{dt}) > 0
        futuresPrices = self.model.getDailyFuturesPrices(self.model.futures{dt},mu,v,lambda)';
...
end % classdef ParticleFilter_SVJDILRL