classdef ParticleFilter_SVJDI < handle
  properties
  ...
  methods
    function self = ParticleFilter_SVJDI(model, nb_particles)
      if nargin > 1
        self.nb_particles = nb_particles;
      end
      self.model          = model;
      self.getParticles();
    end % ParticleFilter_SVJDI
    ...
    function [logLikelihood, vf, lambdaf, Npf, Nnf, Jpf, Jnf, Jvf, Jlambdaf] = logLikelihood(self)
      tic();
      ...
      for dt = 1:T
        [weights, v, lambda, Np, Nn, Jp, Jn, Jv, Jlambda] = self.updateState(pv, dt, v, lambda);
        ...
        switch self.resampling
          case 'MalikPitt'
            [v, lambda] = self.MalikPitt(dt, weights, v, lambda);
          case 'Stratified'
            [v, lambda] = self.Stratified(dt, weights, v, lambda);
          otherwise
    ...
    function [vnew, lambdanew] = Stratified(self, dt, weights, v, lambda)
      I = randsample_stratified_cpp(weights, self.particles.u_resampl(dt,1));
      vnew = v(I);
      lambdanew = lambda(I);
    end % Stratified

    function [vnew, lambdanew] = MalikPitt(self, dt, weights, v, lambda)
      Vnew = NaN(self.nb_particles,2);
      V = [v, lambda];
    ...
    function [weights, v, lambda, Np, Nn, Jp, Jn, Jv, Jlambda] = updateState(self, pv, dt, v_old, lambda_old)
      h           = self.model.h;
      ...
      if self.model.use_futures && height(self.model.futures{dt}) > 0
        futuresPrices = self.model.getDailyFuturesPrices(self.model.futures{dt},v,lambda)';
...
end % classdef ParticleFilter_SVJDI