...
pf = ParticleFilter_SVJDILRL(model,25000);

if OPTIM
  res = model.fminsearch(model.penalty,pf);
  save('results/model_PQ_SVJDILRL','model');

  [logLikelihood, muf, vf, lambdaf, Npf, Nnf, Jpf, Jnf, Jvf, Jlambdaf] = pf.logLikelihood();
  save('results/model_PQ_SVJDILRL_filtered','logLikelihood', 'muf', 'vf', 'lambdaf', 'Npf', 'Nnf', 'Jpf', 'Jnf', 'Jvf', 'Jlambdaf');
end
