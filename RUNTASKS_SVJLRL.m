...
pf = ParticleFilter_SVJLRL(model,25000);

if OPTIM
  res = model.fminsearch(model.penalty,pf);
  save('results/model_PQ_SVJLRL','model');

  [logLikelihood, muf, vf, Npf, Nnf, Jpf, Jnf, Jvf] = pf.logLikelihood();
  save('results/model_PQ_SVJLRL_filtered','logLikelihood', 'muf', 'vf', 'Npf', 'Nnf', 'Jpf', 'Jnf', 'Jvf');
end
