classdef ParticleFilter_SVJLRL < handle
  properties
  ...
  methods
    function self = ParticleFilter_SVJLRL(model, nb_particles)
      if nargin > 1
        self.nb_particles = nb_particles;
      end
      self.model          = model;
      self.getParticles();
    end % ParticleFilter_SVJLRL
    ...
    function [] = getParticles(self)
      ...
      switch self.model.name
        case 'SVJLRL'
          ...
          self.particles.u_resampl  = unifrnd(0,1,T,2);
        otherwise
          error('Model not implemented in the ParticleFilter class')
      end
    end % getParticles
    
    function [logLikelihood, muf, vf, Npf, Nnf, Jpf, Jnf, Jvf] = logLikelihood(self)
      tic();
      ...
      for dt = 1:T
        [weights, mu, v, Np, Nn, Jp, Jn, Jv] = self.updateState(pv, dt, mu, v);
        ...
        switch self.resampling
          case 'MalikPitt'
            [mu, v] = self.MalikPitt(dt, weights, mu, v);
          case 'Stratified'
            [mu, v] = self.Stratified(dt, weights, mu, v);
          otherwise
    ...
    function [munew, vnew] = Stratified(self, dt, weights, mu, v)
      I = randsample_stratified_cpp(weights, self.particles.u_resampl(dt,1));
      munew = mu(I);
      vnew = v(I);
    end % Stratified

    function [munew, vnew] = MalikPitt(self, dt, weights, mu, v)
    ...
    function [weights, mu, v, Np, Nn, Jp, Jn, Jv] = updateState(self, pv, dt, mu_old, v_old)
      h           = self.model.h;
      ...
      if self.model.use_futures && height(self.model.futures{dt}) > 0
        futuresPrices = self.model.getDailyFuturesPrices(self.model.futures{dt},mu,v)';
...
end % classdef ParticleFilter_MRSV