...
%     nlag = lag length to use -- default: floor( 4*(nobs/100)^0.25 )
%---------------------------------------------------
% RETURNS: a structure
%        results.meth  = 'nwest'
%        results.beta  = bhat
...
% Originial version (in original/nwest.m) by:
...
% Modifications: Christian Dorion (christian.dorion@hec.ca)

[nobs nvar] = size(x);

if nargin < 2; error('Wrong # of arguments to nwest'); end;

%CD - nlag: default value using NW formula
if nargin < 3
  nlag = floor( 4*(nobs/100)^0.25 );
end
