...
% Create an instance of the SVJ model
model = SVJ('calendar',Date, ...
             'series',x, ...
...
pf = ParticleFilter_SVJ(model,25000);

if OPTIM
  res = model.fminsearch(model.penalty,pf);
  save('results/model_PQ_SVJ','model');

  [logLikelihood, vf, Npf, Nnf, Jpf, Jnf,Jvf] = pf.logLikelihood();
  save('results/model_PQ_SVJ_filtered','logLikelihood', 'vf', 'Npf', 'Nnf', 'Jpf', 'Jnf', 'Jvf', 'Jlambdaf');
end
